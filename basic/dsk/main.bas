...
2599return
2600ift3<>39thenpx=px+pv:ifpx>246thenpx=246
2610swapp(0),p(1):ps=p(1)
2630return
2640ift7<>39thenpx=px-pv:ifpx<=0thenpx=0
2650swapp(2),p(3):ps=p(3)
2660return
2700ift0<>69andpa=0thenpo=py:pa=1elsepy=py-pv
2750return
...
8020return
9000'nada'
9010preset(0,0):print#1,"px"px"py"py"pa"pa"tx"tx"ty"ty
...
20140pb=2
20190return
10000px=3*8:py=19*8:pv=8:pw=8:pj=0:po=py:pe=100:pa=0:pl=3:pr=0:pb=0:pg=100
...
10120ifpa=1andpy<po-32thenpa=2
10130ifpa=2thenpy=py+pw
10140ifpy>170thenpy=170
10150ifpy<=0thenpy=0
10160ift0=67thenbeep:copy(0,0)-(8,8),1to(px+4,py+8),0:m(ty+1,tx)=-1:pb=pb-1:gosub8000
10170'ifpa=0andt5<>32thenpy=py+pv
10175ift5=32andpa=2thenpy=po:pw=0:pw=-pw:pa=0
10180'ifpa=1andt5=32thenpa=0:strig(0)on
10190'ifpa=1andt1=32thenpy=py-pv
10195return
...
13570return
13600tx=px/8:ty=py/8
13605ifty>0thent0=m(ty+1,tx)
13610't1=m(ty-1,tx)
13620t3=m(ty,tx+1)
13630ifty>0thent5=m(ty+2,tx)
